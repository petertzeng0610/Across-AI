...
import { Eye, Plus, Search, Edit, Download, Calendar, ArrowLeft, Building2, Globe, Activity, Zap, LogOut, Users } from "lucide-react"
...
import { logout, switchToUserContract } from "@/app/routes/auth"
import { getContractsInfoByReseller, getLogs, getPlans, createContract } from "@/app/routes/contracts"
import { createUser, updateUser } from "@/app/routes/users"
...
function DealerManagement() {
  ...
  const [endUsersLength, setEndUsersLength] = useState<any>(0)
  ...
  const [currentView, setCurrentView] = useState<"list" | "detail" | "userLogs">("list")
  const [logs, setLogs] = useState<any>([])
  const [selectedLogs, setSelectedLogs] = useState<any>([])
  const [plans, setPlans] = useState<any>([])
  const [trafficTotal, setTrafficTotal] = useState<any>(0)
  const [requestTotal, setRequestTotal] = useState<any>(0)
  const [totalDomains, setTotalDomains] = useState<any>(0)
  ...
  const loadSettings = async () => {
    const reseller = auth.user;
    setReseller(reseller)
    // 取得訂單列表
    const contractResp = await getContractsInfoByReseller(reseller.userId, reseller.email)
    ...
    contracts.forEach((contract: any) => {
      ...
      newContract.status = active ? 'active' : 'inactive'
      contractTotal.push(newContract)
      totalDomains += contract.zones.length
      endUsersLength += contract.users.length
    })
    setTotalDomains(totalDomains)
    setEndUsersLength(endUsersLength)

    const trafficTotal = contracts.reduce((sum: any, contract: any) => {
      if (!contract.totalTrafficByContract) return sum;
      return sum + contract.totalTrafficByContract;
    }, 0)
    setTrafficTotal(trafficTotal)

    const requestTotal = contracts.reduce((sum: any, contract: any) => {
    ...
    if (selectedContract) {
      const updatedContract = contracts?.find((c: any) => c.contractNo === selectedContract.contractNo)
      if (updatedContract) {
  ...
  const generateMonthlyUsageData = (contracts: any) => {
  ...
  // 生成月度用量數據
  const monthlyUsageData: any = generateMonthlyUsageData(contracts)

  // 獲取當月數據
  const getCurrentMonthData = () => {
    const currentMonth = new Date().toISOString().slice(0, 7) // 格式: YYYY-MM
    const currentMonthData = monthlyUsageData.find((data: any) => data.month === currentMonth)
  ...
  const getHistoricalMonthlyData = () => {
    const currentMonth = new Date().toISOString().slice(0, 7) // 格式: YYYY-MM
    return monthlyUsageData.filter((data: any) => data.month !== currentMonth)
  }
  ...
  // 過濾 contract 數據
  const filteredContracts = contracts.filter((item: any) =>
  ...
  const handleCreateContractSubmit = async (e: React.FormEvent) => {
    ...
    if (contractForm.contractNo && contractForm.plan) {
      setIsLoading(true)
      try {
        await createContract({ ...contractForm, resellerId: reseller?.userId, email: reseller?.email })
      } catch (error: any) {
        console.error(error)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 pt-20">
      <div className="container mx-auto p-6 space-y-8">
        ...
        {currentView === "list" ? (
          <>
            <Card className="bg-gradient-to-r backdrop-blur-sm border-blue-300/30 [&>*]:!bg-transparent">
              <CardContent className="p-8">
                <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
                  <div className="flex-1 space-y-6">
                    <div>
                      <h1 className="text-4xl font-bold text-white mb-2">{reseller.userId} 您好！</h1>
                      <p className="text-blue-200 text-lg">歡迎使用 ADAS ONE 經銷商管理系統</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-blue-300/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-blue-200 text-sm">域名數量</p>
                            <AnimatedCounter
                              value={totalDomains}
                      ...
                      <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-green-300/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-green-200 text-sm">總流量</p>
                            <p className="text-white text-2xl font-bold">
                              <AnimatedCounter
                                value={Number(formatTraffic(trafficTotal).value || 0)}
                                className="text-white text-2xl font-bold"
                                duration={1200}
                              />
                              <span>{formatTraffic(trafficTotal).unit}</span>
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-purple-300/20">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-purple-200 text-sm">總求次數</p>
                            <AnimatedCounter
                              value={Number(formatTraffic(requestTotal).value || 0)}
                              className="text-white text-2xl font-bold"
                              duration={1200}
                            />
                            <span className="text-white text-2xl font-bold">{formatTraffic(requestTotal).unit}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex-shrink-0">
                    <div className="w-64 h-64 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full flex items-center justify-center border border-blue-300/30">
                      <Building2 className="w-32 h-32 text-blue-300/60" />
                    </div>
                  </div>
                </div>
              </CardContent>
              ...
              <CardHeader>
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    ...
                    <CardDescription className="text-blue-200">填寫以下資訊來新增訂單</CardDescription>
                  </div>
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="bg-blue-600/20 backdrop-blur-sm border border-blue-400/30 rounded-lg px-4 py-3 hover:bg-blue-600/30 transition-colors duration-300">
                      <div className="text-center">
                        ...
                        <AnimatedCounter value={endUsersLength} className="text-white text-2xl font-bold" duration={800} />
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setContractForm((prev: any) => ({ ...prev, contractNo: e.target.value }))}
                        placeholder="輸入訂單編號"
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setContractForm((prev: any) => ({ ...prev, plan: value }))}
                      >
                        ...
                        <SelectContent className="bg-slate-800/95 backdrop-blur-sm border-blue-300/20">
                          {plans.map((plan: any) => (
                            <SelectItem key={plan.plan_code} value={plan.plan_code} className="text-white hover:bg-white/10">
                              {plan.name}
                            </SelectItem>
                           ))}
                         </SelectContent>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setContractForm((prev: any) => ({ ...prev, serviceCount: value }))}
                      >
                        <SelectTrigger className="bg-white/10 border-blue-300/30 text-white">
              ...
              <CardHeader>
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    ...
                    <CardDescription className="text-blue-200">管理所有訂單資訊</CardDescription>
                  </div>
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-blue-300 w-4 h-4" />
                    <Input
                      placeholder="搜尋訂單編號或公司名稱..."
                      value={searchTerm}
                ...
                <div>
                  ...
                  <div className="rounded-md border border-blue-300/20 overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        {historicalMonthlyData.map((usage: any, index: number) => (
                      ...
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          ...
                          <div className="flex gap-2">
                            <Button onClick={handleCreateContractSubmit} className="bg-green-600 hover:bg-green-700">
                              儲存
                            </Button>
                            <Button
          ...
          <div className="space-y-6">
            <p className="text-blue-200">此功能正在開發中...</p>
          </div>
        )}
      </div>
