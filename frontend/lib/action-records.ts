...
// 根據平台篩選記錄
export function getActionRecordsByPlatform(platform: "cloudflare" | "f5"): ActionRecord[] {
...
export function getActionStatistics() {
  ...
  return {
    ...
    totalResolved: records.reduce((sum, r) => sum + r.results.resolvedCount, 0),
    totalUnresolved: records.reduce((sum, r) => sum + r.results.unresolvedCount, 0),
