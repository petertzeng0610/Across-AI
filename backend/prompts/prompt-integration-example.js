...
/**
 * Cloudflare WAF 分析 Prompt 整合範例
 * 
 * 此檔案展示如何整合動態 Prompt 到後端 API
 */
...
/**
 * 生成動態 Prompt
 * @param {Object} analysisData - 從 CloudflareWAFRiskService 獲取的分析數據
 * @returns {String} - 完整的 AI Prompt
 */
function generateDynamicPrompt(analysisData) {
  const {
    sqlInjection,
    xssAttacks,
    rceAttacks,
...
4. 每個風險至少提供 2-3 個具體建議
5. aiInsight 必須包含具體數字、WAF 分數、Top 來源、Top 目標
6. 描述要具體提到檢測到的攻擊特徵和 OWASP 分類
...
async function example() {
  // 假設已經從 CloudflareWAFRiskService 獲取了分析數據
  const analysisData = {
    sqlInjection: {
      count: 3120,
      highRisk: 856,
      ...
      affectedAssets: 45,
      topIPs: [
        { item: '195.191.171.12', count: 450 },
        { item: '200.221.12.45', count: 320 }
      ],
      topCountries: [
        { item: 'RU', count: 1240 },
        { item: 'BR', count: 850 }
      ...
      topTargets: [
        { item: '/api/login', count: 1250 },
        { item: '/admin/query.php', count: 890 }
      ]
    },
    xssAttacks: {
      count: 1890,
      ...
      affectedAssets: 23,
      topIPs: [
        { item: '103.45.67.89', count: 180 }
      ],
      topCountries: [
        { item: 'VN', count: 420 }
      ],
      topTargets: [
        { item: '/search', count: 560 }
      ]
    },
    rceAttacks: { count: 0 },  // 沒有 RCE 攻擊，不會出現在 prompt 中
    botTraffic: { count: 0 },
    pathTraversal: { count: 0 },
    abnormalUA: { count: 0 },
    geoAnalysis: {
      topCountries: [
        { item: 'RU', count: 2340 },
    ...
    assetAnalysis: {
      totalAssets: 68,
      topAssets: [
        { item: 'api.example.com', count: 2450 }
      ]
    },
    totalEvents: 15234,
    timeRange: {
  ...
  // 生成動態 Prompt
  const prompt = generateDynamicPrompt(analysisData);
  
  console.log('========== 生成的 Prompt ==========');
  console.log(prompt);
  console.log('===================================');

  // 在實際使用中，將 prompt 發送給 AI
  // const aiResponse = await callAI(prompt);
  // const risks = JSON.parse(aiResponse);
  // return risks;
}
